...
# Split vertically to create pane 0 (left) and pane 1 (right)
tmux split-window -h -t "$SESSION:$WINDOW_INDEX.0"

# Split pane 0 horizontally to create pane 0 (top) and pane 2 (bottom)
tmux split-window -v -t "$SESSION:$WINDOW_INDEX.0"

# Split pane 1 horizontally to create pane 1 (top) and pane 3 (bottom)
tmux split-window -v -t "$SESSION:$WINDOW_INDEX.1"
...
send_command() {
    ...
    tmux send-keys -t "$SESSION:$WINDOW_INDEX.$pane" "$command" C-m
}
...
# send_command 0 "cd $WORKING_DIR && docker-compose restart && docker exec -it deepseek_ica_agent_geomates_1  sbcl --script geomates.lisp"
# send_command 0 "cd $WORKING_DIR && docker-compose restart && docker exec -it deepseek_ica_agent_geomates_1   sbcl --script geomates.lisp"
send_command 0 "cd $WORKING_DIR && docker exec -it deepseek_ica_agent_geomates_1   sbcl --script geomates.lisp"
