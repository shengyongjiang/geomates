...
;;; 2023.05.16 Dan
;;;             : * Use exp-coerced and log-coerced for the error handling.
;;; 2023.05.24 Dan [8.2a2]
;;;             : * Specify :required :vision in the module definition so that
;;;             :   once it's loaded it is available whenever the :vision 
;;;             :   module is "used".
;;; 2024.07.08 Dan [8.2a3]
;;;             : * Removed some unnecessary scheduling of state changes and
;;;             :   make the changes in the appropriate actions (which are
;;;             :   already scheduled) so the state is consistent with how a
;;;             :   motor action progresses.
;;;             : * When a vision clear happens check the current state of the
;;;             :   emma action and use the general clear action if it's safe
;;;             :   to stop the ongoing eye movement.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defun emma-clear-action ()
  (let ((emma (get-module :emma)))
    (when emma 
      (bt:with-recursive-lock-held ((param-lock emma))
        (when (enabled emma) 
          ;; can't stop current eye movement but if still in prep remove it
...
(defmethod initiate-eye-move ((emma emma-vis-mod) (recog-time number) (sacc saccade))
  (bt:with-recursive-lock-held ((param-lock emma))
    (let ()
      ...
      ;; to be consistent with motor make both busy here
      (change-state emma :prep 'busy :proc 'BUSY)
      
      ; don't schedule this because we want it
      ; to happen at the same time as the initial
      ; request (schedule-event-now 'change-state :params (list emma :prep 'busy) :module :emma :output nil :priority :max)
  ...
  (let ((freq (object-frequency emma obj))
    ...
    (bt:with-recursive-lock-held ((param-lock emma))
      (randomize-time (* (enc-factor emma)
                         (- (log-coerced freq))
                         (exp-coerced (* (r sacc) (enc-exponent emma))))))))
