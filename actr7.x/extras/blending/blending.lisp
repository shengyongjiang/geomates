...
;;; 2023.05.16 Dan
;;;             : * Use exp-coerced and expt-coerced for the error handling.
;;; 2024.03.14 Dan
;;;             : * Updated the trace to actually show the chunks from DM that
;;;             :   matched the common slots in the chunks case that's not all
;;;             :   of DM.
;;; 2024.05.03 Dan
;;;             : * Fixed a bug with how that previous update was applied to the
;;;             :   output for the Environment tool.
;;; 2024.05.18 Dan
;;;             : * Sort the slots to be blended by index so that it is
;;;             :   consistent across Lisps.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
    ...
    (let ((ignore nil) 
      ...
      (bt:with-recursive-lock-held ((blending-module-lock instance))
        ...
        (let* ((dm (get-module declarative))                   ;; get that module since we're using some of its functions
               ...
               (blended-slots (sort (remove-if (lambda (x) 
                                                 (find x ignore))
          ...
          (dolist (x activation-list)
            (setf (second x)
              (exp-coerced (/ (- (first x) max-ai) temperature))))
                           ...
                           (when (and (eq which :chunks) (blending-module-trace instance))
                             (if (not (zerop type))
                                 (progn
                                   ...
                                   (model-output "Chunks from DM with those slots: ~s" chunks))
                                 ...
                                 (let ((sum 0.0))
                                   ...
                                   (dolist (possible possible-values)
                                     ...
                                     (let ((sim (similarity-fct val (blending-item-mag possible))))
                                       ...
                                       (incf sum (* (blending-item-prob possible) (expt-coerced sim 2)))
                           ...
                           (when (eq (sblt-slot-condition slot) :chunks)
                             (if (not (zerop (sblt-slot-ctype slot)))
                                 (progn
                                   ...
                                   (command-output "Chunks from DM with those slots: ~s" (sblt-slot-chunks slot)))
               ...
               (let ((mags (blending-history-data-item "magnitudes" (second slot))))
                 ...
                 (aif (blending-history-data-item "chunk-type-slots" (second slot))
                      (progn
                        ...
                        (format s " Chunks from DM with those slots: (~{~a~^ ~})~%" (mapcar 'first mags)))
                     ...
                     (let ((sum 0.0))
                       ...
                       (dolist (possible-list (blending-history-data-item "details" (second val)))
                         (let ((name (blending-history-data-item "chunk" possible-list))
                           ...
                           (incf sum (* probability (expt-coerced sim 2)))
