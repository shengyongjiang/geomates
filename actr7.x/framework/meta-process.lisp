...
;;; 2024.04.30 Dan
;;;             : * Adding the with-running-or-defining-lock macro that is 
;;;             :   similar to the with-top-level-lock macro to protect things
;;;             :   instead of the two separate defining and running locks that
;;;             :   were being used before.
;;;             : * Updated mp-running, reset-mp, and mp-real-time-managment
;;;             :   to use with-running-or-defining-lock.
;;; 2024.05.15 Dan
;;;             : * Fixed insert-waiting-event because just setting the event
;;;             :   max slot isn't good enough since min is also set whenever
;;;             :   that gets called, so min needs to be cleared.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
...
(defun reset-mp (meta-process)
  ...
  ;; (clear-mp-running) must be true to get here -- only called by clear-all and internal-reset
...
(defun mp-running? ()
  (awhen (current-mp)
         (bt:with-lock-held ((meta-p-run-lock it))
           (eq (meta-p-running it) :running))))

(add-act-r-command "act-r-running-p" 'mp-running? "Predicate to check whether or not the system is currently running. No params.")

#|

   Replacing these with a construct similar to
   with-top-level-lock since meta-p-running
   is protected by the meta-p-run-lock
   and since the define-model-lock is basically
   protecting the same things just use a 
   running-or-defining blocker like the top-level
   lock using the meta-p-running flag.
...
(defmacro with-running-or-defining-lock (cmd which &body body)
  (declare (ignore cmd which))
  `(progn
     ,@body))
...
(defmacro with-running-or-defining-lock (cmd which &body body)
  `(let* ((it (current-mp))
          (blocked (bt:with-lock-held ((meta-p-run-lock it))
                    (if (meta-p-running it)
                        (meta-p-running it)
                      (progn 
                        (setf (meta-p-running it) ,which)
                        nil)))))
     (if blocked
         (print-warning "~a cannot be used because ACT-R is currently ~a" ,cmd blocked)
       (unwind-protect 
           (progn
             ,@body)
         (bt:with-lock-held ((meta-p-run-lock it))
...
(defun mp-real-time-management (&key (mode 'interval)
  ...
  (verify-current-mp  
   ...
   (with-running-or-defining-lock "mp-real-time-management" "configuring real-time settings"
    ...
    (let ((matching-event (find-if (lambda (x)
      ...
      (cond (matching-event 
            ...
            ((null delay)
             ...
             (setf (act-r-event-min new-event) nil)
             (setf (act-r-event-wait-condition new-event) nil)
