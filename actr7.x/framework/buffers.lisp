...
;;; 2023.05.19 Dan [7.2]
;;;             : * Added the reusable-buffer-p function that can be used to
;;;             :   check if a buffer has the copy mechanism enabled.
;;;             : * Added a remote buffer-requires-copies command.
;;; 2023.06.30 Dan
;;;             : * Buffer-chunk now sorts the list of buffers if no buffers 
;;;             :   are specified.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
...
(defun external-buffer-requires-copies (buffer-name)
  (buffer-requires-copies (string->name buffer-name)))

(add-act-r-command "buffer-requires-copies" 'external-buffer-requires-copies "Set the named buffer to not reuse a single buffer chunk. Params: buffer" nil)

(defun reusable-buffer-p (buffer-name)
  (let ((buffer (buffer-instance buffer-name)))
    (if (null buffer)
        (print-warning "reusable-buffer-p called with an invalid buffer name ~S" buffer-name)
        
      (bt:with-recursive-lock-held ((act-r-buffer-lock buffer))
        (if (act-r-buffer-reuse? buffer) 
            t
          nil)))))

(defun external-reusable-buffer-p (buffer-name)
  (multi-buffer-p (string->name buffer-name)))

(add-act-r-command "reusable-buffer-p" 'external-reusable-buffer-p "Check if the named buffer is using a single buffer chunk. Params: buffer" nil)
