...
;;; 2024.04.23 Dan [5.3]
;;;             : * Changed how the locks are handled around the received-worker
;;;             :   to clean-up some issues and fix a problem with a potential
;;;             :   race condition that seems to only show up in the new SBCL
;;;             :   (with the futexes instead of mutexes).
;;;             : * Handle the action evaluation differently based on the 
;;;             :   single-instance setting.  Without it don't need the lock (it
;;;             :   isn't created now), with t just hold the lock while it is
;;;             :   being evaluated and block on next call (like before), and
;;;             :   with a string it uses the lock to protect a 'running' flag
;;;             :   so that it can use with-lock-held instead of an acquire test
;;;             :   since SBCL says acquire (grab) isn't safe on its own and
;;;             :   bordeaux threads just calls it directly.
;;;             : * Changed with-top-level-lock to use *top-level-user* as a
;;;             :   flag to check to avoid the acquire-lock test.
;;; 2024.04.29 Dan
;;;             : * Properly indented and removed some of the 'testing' code
;;;             :   from the things related to that last update.
;;; 2024.05.01 Dan
;;;             : * Removed the acquire-lock from send-remote-command and use
;;;             :   a with-lock-held instead.
;;; 2024.05.15 Dan
;;;             : * Fixed a problem with how execute-command returned some error
;;;             :   results.  It was returning multiple values for some errors,
;;;             :   but it returned a list for success so the code that was
;;;             :   using that didn't get all of the error messages.
;;;             : * Cleaned up call-act-r-command since it can just use the
;;;             :   handle-evaluate-results macro.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
...
(defstruct dispatch-command
  ...
  ;; these are lists of dispatch-command structures
  before 
  after 
  simple
  monitoring
          ...
          (bt:with-recursive-lock-held ((dispatcher-command-lock *dispatcher*))
            (let ((current (gethash name (dispatcher-command-table *dispatcher*))))
              (if current
                ...
                (let ((c (if single-instance
                             (make-dispatch-command :name name :underlying-function function
                                                    :documentation documentation 
                                                    :evaluator evaluator
                                                    :single-instance single-instance
                                                    :local-name local-name
                                                    :encoded encoded)
                           ;; don't make the lock if it's not needed
                           (make-dispatch-command :name name :underlying-function function
                                                  :documentation documentation 
                                                  :evaluator evaluator
                                                  :single-instance single-instance
  ...
  (handler-case
      (if (not (stringp name))
        ...
        (let ((c (bt:with-recursive-lock-held ((dispatcher-command-lock *dispatcher*))
          ...
          (if (null c)
              (list nil (format nil "Command ~s not found in the table and cannot be executed." name))
            ...
            (let (before after simple)
              ...
              ;;; to avoid the repeated code use an flet
              
              (flet ((process-command ()
                                      (dolist (x before)
                ...
                ;;; Avoid the separate acquire and release and just split on whether
                ;;; it's single instance right from the start.  
                             
                (cond ((null (dispatch-command-single-instance c))
                       (process-command))
                      
                      ((eq t (dispatch-command-single-instance c))
                       (bt:with-lock-held ((dispatch-command-lock c))
                         (process-command)))
                      
                      (t ;; it's a string so need to just quit if already running
                       
                       (bt:with-lock-held ((dispatch-command-lock c))
                         (if (dispatch-command-running c)
                             (progn
                               (return-from execute-command
                                 (list nil (dispatch-command-single-instance c))))
                           (progn
                             (setf (dispatch-command-running c) t))))
                       (unwind-protect
                           (process-command)
                         (progn
                           (bt:with-lock-held ((dispatch-command-lock c))
                             (setf (dispatch-command-running c) nil)))))))))))
...
;;; but there's a problem with doing that since it gets called during
;;; a reload. A recursive lock might solve that, but perhaps also allows
;;; for other probelms.  So for now, clear-all isn't protected by this
;;; but since it also isn't made available remotely that's probably not
;;; a problem.
...
#-:single-threaded-act-r
(defmacro with-top-level-lock (warning user &body body)
  `(let ((blocked (bt:with-lock-held (*top-level-lock*)
                    (if *top-level-user*
                        *top-level-user*
                      (progn 
                        ...
                        nil)))))
     (if blocked
         (print-warning "ACT-R system unavailable because of ~a. ~a" blocked ,warning)
       (unwind-protect 
           (progn
             ,@body)
         (bt:with-lock-held (*top-level-lock*)
           (setf *top-level-user* nil))))))
...
(defun call-act-r-command (command &rest rest)
  (handle-evaluate-results (apply 'evaluate-act-r-command (cons command rest))))
