...
;;; 2023.02.15 Dan
;;;             : * Rewrote vv-handle-click to use with-recursive-lock-held
;;;             :   instead of acquire and release because of the potential
;;;             :   issues with those with SBCL (and maybe other systems).  
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (let ((handled nil)
    ...
    (bt:with-recursive-lock-held ((window-lock vw))
      (when (point-in-vv-p vw pos)
        (setf local-pos (map 'vector '- pos (vector (x-pos vw) (y-pos vw))))
        (do* ((subviews (subviews vw) (cdr subviews))
              (sub (car subviews) (car subviews)))
             ((or handled (null sub)))
          (when (and (handles-click-p sub)
                     (point-in-vv-p sub local-pos))
            (setf handled sub)))))
    (when handled 
      (vv-click-event-handler handled local-pos)
      t)))
