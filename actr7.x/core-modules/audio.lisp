...
;;; 2024.04.01 Dan 
;;;             : * When a chunk doesn't get stuffed into aural-location because
;;;             :   of a spec set with set-audloc-default it should not schedule
;;;             :   the find-sound-failure event, and this fixes that.
;;;             : * And, move stuff-sound-buffer back to a maintenance event
;;;             :   because issues about rescheduling conflict-resolution were
;;;             :   fixed a long time ago...
;;;             : * Of course, that means the time will still advance to the
;;;             :   time of the stuff-sound-buffer action without a subsequent
;;;             :   conflict resolution if it happens to be at the end of a run,
;;;             :   but that seems like a more minor issue than having the 
;;;             :   model "detect" that there was a sound it shouldn't hear.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (let ((evt (call-next-method)))
    (when evt
      (schedule-event (detect-at-time evt) 'stuff-sound-buffer :module :audio
                      :destination :audio :output nil :maintenance t :time-in-ms t)
