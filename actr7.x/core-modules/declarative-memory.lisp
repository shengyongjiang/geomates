...
;;; 2022.11.28 Dan [8.0]
;;;             : * Adding :bll-value, :mas-value, and :ans-value request 
;;;             :   parameters to the retrieval buffer requests to allow 
;;;             :   changing those parameters so that a request can temporairly
;;;             :   adjust any of the activation mechanisms.  However, setting
;;;             :   them on in the model and turning off or adjusting when 
;;;             :   desired is probably the only safe way to use it since
;;;             :   if a mechanism is not on at model definition time it may
;;;             :   fail to record important information like, reinforcement
;;;             :   references.
;;; 2023.10.03 Dan [8.1]
;;;             : * Added :lf-value to the retrieval requests as well so that
;;;             :   one can adjust that too.
;;; 2024.04.30 Dan
;;;             : * Replace the bt:acquire-lock calls with with-lock-held and
;;;             :   adjust the code a little to avoid multiple calls.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
...
;;;
;;;
;;; In addition, to that, there are request parameters which may be used -
;;; :recently-retrieved, :mp-value, :rt-value, :bll-value, :mas-value, 
;;; :ans-value, and :lf-value.
;;;
...
;;;
;;; The :*-value request parameters can be used to temporarily change the 
;;; setting of the corresponding module parameters while the request is processed.
;;;
...
(define-module-fct 'declarative 
    (list (define-buffer-fct 'retrieval 
            :request-params (list :recently-retrieved :mp-value :rt-value :bll-value :mas-value :ans-value :lf-value)
