    ...
    if {$stepper_tutored == 1} {
      ...
      foreach i $tutored_display {
        ...
        while {[set indx [.stepper.prod_frame.f3.f.text search $match $strt end]] != ""} { 
          ...
          .stepper.prod_frame.f3.f.text tag configure $t_name -background gray -foreground white 
...
proc check_conflict_resolution {} {
  ...
  if {$tutor_cr_none == 0 && $picked == 0} {
   ...
   tk_messageBox -icon info -type ok -title "Tutoring" \
    ...
    .tutor_cr.frame.c yview moveto 0
...
proc display_conflict_resolution {} {
  ...
  if {[winfo exists .tutor_cr] == 1} {
    ...
    message .tutor_cr.message \
          -text "Using the information in the buffers, click the box next to each production condition that matches the current state, or check the box labeled 'None match' if none of them match.  Then Press the Check button when done." \
          -font label_font
    ...
    foreach i $data {
      ...
      label .tutor_cr.frame.c.contents.$count.but.text  -font text_font -width 45 -height [expr 1 + [llength [split [lindex $i 0] "\n"]]] -justify left -text "(p production[expr 1 + $count]\n[lindex $i 0]==>"
      ...
      pack .tutor_cr.frame.c.contents.$count.but.text -side left
...
proc check_start_retrieval {} {
  ...
  if {$tutor_r_none == 0 && $picked == 0} {
   ...
   tk_messageBox -icon info -type ok -title "Tutoring" \
    ...
    .tutor_r.frame.c yview moveto 0
...
proc display_retrieval_choice {} {
  ...
  if {[winfo exists .tutor_r] == 1} {
    ...
    foreach i [lindex $data 1] {
      ...
      label .tutor_r.frame.c.contents.$count.but.text -font text_font -width 45 -height [llength [split [lindex $i 0] "\n"]] -justify left -text [lindex $i 0]
