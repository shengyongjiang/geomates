...
;;;             :   the correct program.
;;; 2023.06.27 Dan
;;;             : * Added a run-environment command for macOS with SBCL that
;;;             :   depends on asdf's launch-program.
;;; 2023.07.17 Dan
;;;             : * Changed all of the mac run-environment calls to use the
;;;             :   command script that picks the application based on the
;;;             :   processor.
;;; 2024.06.18 Dan
;;;             : * Updated the macOS SBCL run-environment and added ones
...
(defun run-environment ()
  (let ((c (ccl::cd "."))) 
    (ccl::cd "ACT-R:environment")
    (run-program "./run-mac-environment.command" nil :wait nil)
    (ccl::cd c)))
...
(defun run-environment ()
  (sys:call-system (format nil "'~a/run-mac-environment.command'"
...
(defun run-environment ()
  (let ((c (current-directory)))
    (chdir "ACT-R:environment")
    (run-shell-command "./run-mac-environment.command" :wait nil)
    (chdir c)))

#+(and :asdf :sbcl :os-macosx)
(defun run-environment()
  (uiop::launch-program (list (translate-logical-pathname "ACT-R:environment;run-mac-environment.command"))))

#+(and :asdf :sbcl :windows)
(defun run-environment ()
  (uiop::launch-program (list (merge-pathnames (translate-logical-pathname "ACT-R:environment;") "Start Environment.exe"))))

#+(and :asdf :sbcl :linux)
(defun run-environment ()
  (uiop::launch-program (list (merge-pathnames (translate-logical-pathname "ACT-R:environment;") "start-environment-Linux"))))
